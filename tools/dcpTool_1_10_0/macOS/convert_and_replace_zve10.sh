...
for DCP_FILE in "$IN_FOLDER_DCP"/*.dcp; do
    ...
    # 执行dcptool命令进行转换
    /mnt/e/data/dcp_change_tool/dcpTool_1_10_0/Binaries/macOS/dcptool -d "$DCP_FILE" "$OUTPUT_FILE_XML"
done
...
for XML_FILE in "$IN_FOLDER_XML"/*.xml; do
    ...
    if [ -f "$XML_FILE" ]; then
        ...
        # 执行dcptool命令进行转换
        /mnt/e/data/dcp_change_tool/dcpTool_1_10_0/Binaries/macOS/dcptool -c "$XML_FILE" "$OUTPUT_FILE_DCP"
    fi
done
